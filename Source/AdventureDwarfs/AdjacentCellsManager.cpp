...
{
	const int ParentLocationX = parentCell->OriginalLocation.X;
	const int ParentLocationY = parentCell->OriginalLocation.Y;
	const int halfSize = parentCell->CellMesh->GetStaticMesh()->GetBounds().BoxExtent.X*2;
...
void AdjacentCellsManager::GetAdjacentGridPos(GridPosition& GridPosition, AdjecantDirections DirectionToGet)
{
	...
	rowResult =  parentCell->Row + rowModifier;
	columnResult = parentCell->Column + columnModifier;
	...
	UE_LOG(LogTemp, Log, TEXT("%d : this column: %d AND this row: %d"), DirectionToGet, parentCell->Column, parentCell->Row);
...
bool AdjacentCellsManager::RaycastAdjacentObjects(int posX, int posY, FHitResult& result, FVector componentUpVector, UWorld* componentWorld)
{
